...
class Helpers {
	...
	public static function locate_template( string $tpl ) {
		...
		$path = apply_filters( 'beapi_helpers_locate_template_templates', [ 'views/' . TRASH_LOG_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ], $tpl, __NAMESPACE__ );
		...
		if ( is_file( TRASH_LOG_DIR . 'views/' . $tpl . '.php' ) ) {
			return ( TRASH_LOG_DIR . 'views/' . $tpl . '.php' );
		}
	 ...
	 * @return string Formatted size in human readable format, or empty string if option doesn't exist.
	 */
	public static function get_option_size( string $option_name ): string {
